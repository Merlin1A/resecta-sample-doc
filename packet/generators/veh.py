...
def draw(rc, base_page: int) -> None:
    ...
    rc.text(
        L.LEFT,
        y,
        "Memo Regarding: the enclosed certificate of title submitted with this file.",
        L.REG,
        9,
        L.GRAY,
    )
    y -= 13
    rc.text(
        L.LEFT, y, "Attachment: one certificate of title enclosed with this file.", L.REG, 9, L.GRAY
    )
    y -= 13
    rc.text(
        L.LEFT,
        y,
        "Reminder: Confidential -- handle this enclosed attachment per the cover memo.",
        L.REG,
        9,
        L.GRAY,
    )
    ...
    rc.text(
        L.LEFT + 200, y, "Model Year: 2019", L.MED, 9, L.INK
    )  # bare year -> DOB ignores it (C2)
    y -= 15
    ...
    y = L.field_multiline(
        rc, L.LEFT, y, G["occ_veh_03"].label_context, G["occ_veh_03"], vsize=9.5, leading=12
    )
    y -= 6
