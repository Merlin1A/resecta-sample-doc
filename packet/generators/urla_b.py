...
def _page2(rc, base_page):
    rc.begin_page(base_page)
    y = C.continuation_header(
        rc, "Uniform Residential Loan Application -- continued", "Page 2 of 12"
    )
    lx = rc.text(
        L.LEFT, y, G["occ_urlab_02"].label_context, L.MED, 8, L.GRAY
    )  # Borrower (printed):
    ...
    y = C.fine_print(
        ...
        y,
        [
            "Enter each revolving tradeline with the full account identifier shown "
    ...
    y = C.frow(
        rc, y, G["occ_urlab_24"], step=18
    )  # Collateral Vehicle Plate No: (doctype-gated MNF)
