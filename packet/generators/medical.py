...
def draw_h1(rc, base_page: int) -> None:
    ...
    y = L.form_title(
        rc,
        "Health Claim -- Field Structure Specimen",
    ...
    L.box_caption(
        rc, L.LEFT, y - h, L.LEFT + W * 0.50, y, "3. " + G["occ_h1_02"].label_context
    ...
    _box(
        ...
        W * 0.50,
        h,
        "6. " + G["occ_h1_03"].label_context,
        G["occ_h1_03"],
        tail="(patient chart)",
    )  # 6 MRN
    y -= h + 2
    L.box_caption(
        rc, L.LEFT, y - 36, L.LEFT + W * 0.50, y, "5. " + G["occ_h1_09"].label_context
    )  # Patient Address:
    rc.value_multiline(G["occ_h1_09"], L.LEFT + 6, y - 15, L.REG, 9, L.INK, 11)
    _box(
        rc, L.LEFT + W * 0.50, y, W * 0.50, 36, "7. " + G["occ_h1_08"].label_context, G["occ_h1_08"]
    )  # Patient Telephone
    y -= 38
    ...
    _box(
        rc, L.LEFT, y, W * 0.34, h, "17b. " + G["occ_h1_07"].label_context, G["occ_h1_07"]
    )  # Referring NPI (broken)
    _box(
        rc,
        L.LEFT + W * 0.34,
        y,
        W * 0.33,
        h,
        "24J. " + G["occ_h1_05"].label_context,
        G["occ_h1_05"],
    )  # Rendering NPI
    _box(
        rc, L.LEFT + W * 0.67, y, W * 0.33, h, "33. " + G["occ_h1_06"].label_context, G["occ_h1_06"]
    )  # Billing NPI
    y -= h + 2
    L.box_caption(
        rc, L.LEFT, y - h, L.LEFT + W * 0.50, y, "31. " + G["occ_h1_10"].label_context
    ...
    cols = [
        (L.LEFT, "Place"),
        (L.LEFT + 90, "Procedure"),
        (L.LEFT + 180, "Diagnosis pointer"),
        (L.LEFT + 300, "Charge"),
        (L.LEFT + 380, "Days/Units"),
        (L.LEFT + 460, "Emergency"),
    ]
    ...
    y -= 14
    lx = rc.text(
        L.LEFT, y, "21. " + G["occ_h1_12"].label_context, L.MED, 8, L.GRAY
    )  # Diagnosis Code:
    ...
    rc.text(
        L.LEFT,
        y,
        "12. Patient authorization to release payment to the provider -- signed below.",
        L.REG,
        8.5,
        L.GRAY,
    )
    ...
    lx = rc.text(
        L.LEFT + 90, y, G["occ_h1_13"].label_context, L.MED, 8, L.GRAY
    )  # Claim Control No.
    ...
    rc.text(
        L.LEFT,
        y,
        "Specimen prepared for software testing; every person, identifier, and charge shown is "
        "fictional and disclosed.",
        L.REG,
        7.5,
        L.FAINT,
    )
...
def draw_h2(rc, base_page: int) -> None:
    ...
    lx = rc.text(
        L.LEFT, L.TOP - 52, G["occ_h2_10"].label_context, L.MED, 8.5, L.GRAY
    )  # Clinic Phone:
    ...
    lx = rc.text(
        bx0 + 6, L.TOP - 56, G["occ_h2_13"].label_context, L.MED, 7, L.GRAY
    )  # Accession No.
    ...
    rc.text(
        L.LEFT,
        L.TOP - 78,
        "Visit summary generated at the close of the encounter; retain with the clinical file.",
        L.REG,
        8,
        L.GRAY,
    )
    ...
    rc.text(
        L.LEFT,
        y,
        "Vitals: BP 128/78 mmHg, pulse 72, temperature 98.4 F, weight 164 lb.",
        L.REG,
        8.5,
        L.INK,
    )
    ...
    rc.text(
        L.LEFT,
        y,
        "No allergy documented. Neither tablet was changed today; the dosage carried "
        "forward from the last visit.",
        L.REG,
        8,
        L.GRAY,
    )
    ...
    rc.text(
        px + 12,
        y,
        "Visit type: office follow-up, established patient, outpatient clinic.",
        L.REG,
        8.5,
        L.INK,
    )
    ...
    rc.text(
        px + 12,
        y,
        "Enumerated, individual. Next visit in three months or sooner if a symptom returns.",
        L.REG,
        8.5,
        L.INK,
    )
    ...
    rc.text(
        L.LEFT,
        y,
        "signed the summary electronically at the close of the visit.",
        L.REG,
        8.5,
        L.GRAY,
    )
    ...
    rc.text(
        L.LEFT,
        y,
        "Synthetic visit summary prepared for software testing; every person, "
        "identifier, medication, and date shown is fictional and disclosed.",
        L.REG,
        7.5,
        L.FAINT,
    )
