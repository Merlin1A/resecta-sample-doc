...
def draw_m1(rc, base_page: int) -> None:
    ...
    rc.text(L.LEFT, L.TOP - 28, "Regional Processing Center -- Determination Unit", L.REG, 8.5, L.GRAY)
    rc.hrule(L.TOP - 36, L.LEFT, L.RIGHT, L.ACCENT, 1.2)
    y = L.TOP - 52
    L.field(rc, L.LEFT, y, G["occ_m1_07"].label_context, G["occ_m1_07"])            # Date: 08/11/2026
    L.field(rc, L.LEFT + 300, y, G["occ_m1_08"].label_context, G["occ_m1_08"])      # Office Code: 0417-B
    y -= 26
    ...
    lx = rc.text(L.LEFT + 250, y - 6, G["occ_m1_09"].label_context, L.MED, 8, L.GRAY)   # Reference No.
...
def draw_m2(rc, base_page: int) -> None:
    ...
    rc.text(L.LEFT, L.TOP - 28, "Residential electric service -- monthly billing statement", L.REG, 8.5, L.GRAY)
    rc.hrule(L.TOP - 36, L.LEFT, L.RIGHT, L.ACCENT, 1.2)
    y = L.TOP - 52
    ...
    cols = [(L.LEFT, "Previous Reading"), (L.LEFT + 120, "Current Reading"), (L.LEFT + 240, "kWh Used"),
            (L.LEFT + 320, "Rate Schedule"), (L.LEFT + 410, "Billing Days")]
    ...
    y = C.table_header(rc, y, cols)
    for desc, amt in (("Previous balance", "$118.62"), ("Payment received -- thank you", "-$118.62"),
                      ("Energy charge (786 kWh)", "$71.40"), ("Basic service charge", "$12.50"),
                      ("Delivery and transmission", "$44.19"), ("Franchise fee and tax", "$14.08")):
    ...
    rc.c.setDash()
    y -= 16
    rc.text(L.LEFT, y, "Remittance Stub -- detach and return with your payment", L.SEMI, 9, L.ACCENT)
    y -= 16
