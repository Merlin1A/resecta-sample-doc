...
def _line(rc, y, s, *, font=None, size=9, color=None, x=None):
    rc.text(TX if x is None else x, y, s, L.REG if font is None else font, size, L.INK if color is None else color)
...
def draw_k1(rc, base_page: int) -> None:
    ...
    lx = rc.text(TX, ys[0], G["occ_k1_03"].label_context, L.MED, 9, L.GRAY)          # Attorney for Plaintiff:
    ...
    rc.value_multiline(G["occ_k1_04"], TX, ys[3], L.REG, 9.5, L.INK, leading)        # firm address (2 lines)
    ...
    for i, s in ((7, "IN THE DISTRICT COURT OF THE SEVENTH JUDICIAL DISTRICT OF THE STATE OF IDAHO,"),
    ...
    rc.text(px + 4, ys[20], "is an individual who at all relevant times managed the freight", L.REG, 9, L.INK)
    _line(rc, ys[21], "operations of the co-defendant (a domestic corporation) within this judicial district.")
    _line(rc, ys[22], "3.   This court has jurisdiction over the parties and the subject matter, and venue is")
    _line(rc, ys[23], "proper because the events giving rise to the claims occurred within the county.")
    _line(rc, ys[24], "4.   Plaintiff seeks compensatory damages, costs, and such further relief as the court")
    _line(rc, ys[25], "deems just. A jury trial is demanded on every claim so triable.")
    _line(rc, ys[26], "Synthetic pleading prepared for software testing; every party, counsel, and value is",
          size=8, color=L.GRAY)
...
def draw_k2(rc, base_page: int) -> None:
    ...
    lx = rc.text(L.LEFT, L.TOP - 42, G["occ_k2_10"].label_context, L.MED, 8.5, L.GRAY)      # Deposition taken
    px = rc.value(G["occ_k2_10"], lx + 4, L.TOP - 42, L.REG, 9, L.INK)                      # 07/30/2026
    rc.text(px + 6, L.TOP - 42, "-- excerpt, pages 14 to 15 of the certified transcript.", L.REG, 8.5, L.GRAY)
    ...
    rc.text(px + 12, y, "-- duly sworn under oath; counsel for both parties present.", L.REG, 8.5, L.GRAY)
    y -= 22
    ...
    rc.text(tx + 16, ys[13], "No. The dispatch desk sent the paperwork, but the instruction came from him.", L.REG, 9, L.INK)
    q(14, "I am handing you what has been marked as this exhibit. Do you recognize it?")
    rc.text(tx, ys[15], "A.", L.SEMI, 9, L.INK)
    rc.text(tx + 16, ys[15], "Yes. That is the route sheet from that week, with my initials in the corner.", L.REG, 9, L.INK)
    ...
    rc.text(tx, ys[19], "(Whereupon a recess was taken. The deposition resumed as reflected in the", L.REG, 8.5, L.GRAY)
    rc.text(tx, ys[20], "certified transcript. Synthetic exhibit prepared for software testing; every", L.REG, 8.5, L.GRAY)
    rc.text(tx, ys[21], "person and value is fictional and disclosed in the accompanying manifest.)", L.REG, 8.5, L.GRAY)
...
def draw_k3(rc, base_page: int) -> None:
    ...
    rc.text(L.LEFT, L.TOP - 27, "Office of Public Disclosure -- Freedom of Information Program", L.REG, 8.5, L.GRAY)
    rc.value_multiline(G["occ_k3_11"], L.LEFT, L.TOP - 41, L.REG, 8.5, L.GRAY, 11)   # agency address (2 lines)
    rc.hrule(L.TOP - 58, L.LEFT, L.RIGHT, L.ACCENT, 1.2)
    y = L.TOP - 74
    rc.value(G["occ_k3_10"], L.LEFT, y, L.REG, 9, L.INK)                              # August 14, 2026
    lx = rc.text(L.LEFT + 300, y, G["occ_k3_09"].label_context, L.MED, 8.5, L.GRAY)    # FOIA Request No.
    ...
    lx = rc.text(L.LEFT, y, G["occ_k3_08"].label_context, L.MED, 8.5, L.GRAY)      # Exemptions cited: (b)(6) and
    ...
    rc.text(bx + 6, y, P.EXEMPTION_B6, L.REG, 8, L.FAINT)
    y -= 16
    rc.text(L.LEFT + 6, y, "Summary: the complaint alleged that a freight route was altered without notice. "
            "The reviewing", L.REG, 8.5, L.INK)
    y -= 12
    rc.text(L.LEFT + 6, y, "officer found the account credible and referred the matter for further review.",
            L.REG, 8.5, L.INK)
    y -= 18
    ...
    rc.text(px + 8, y, "-- individual, enumerated; roster line released without redaction.", L.REG, 8.5, L.GRAY)
    y -= 10
    ...
    rc.text(L.LEFT, y, "Synthetic release prepared for software testing; every person, identifier, and "
            "request value is fictional and disclosed in the accompanying manifest.", L.REG, 7.5, L.FAINT)
...
def draw_k4(rc, base_page: int) -> None:
    rc.begin_page(base_page)
    y = L.form_title(rc, "Civil Docket and Judgment Sheet",
    ...
    rc.text(px + 12, y, "plus costs; accruing at the statutory rate until satisfied.", L.REG, 8.5, L.GRAY)
    y -= 14
    ...
    rc.text(L.LEFT, y, "Synthetic docket prepared for software testing; every party, sum, date, and account "
            "value is fictional and disclosed.", L.REG, 7.5, L.FAINT)
