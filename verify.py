...
DENYLIST = [
    "CHASE", "JPMORGAN", "BANK OF AMERICA", "BOFA", "WELLS FARGO", "CITIBANK", "CITIGROUP",
    "U.S. BANK", "US BANK", "PNC", "CAPITAL ONE", "TRUIST", "TD BANK", "ALLY", "REGIONS",
    "FIFTH THIRD", "KEYBANK", "SANTANDER", "HSBC", "NAVY FEDERAL", "AMERICAN EXPRESS", "AMEX",
    "DISCOVER", "VISA", "MASTERCARD", "ZELLE", "VENMO", "PAYPAL", "CASHAPP", "TREAS", "IRS", "SSA",
    "FDIC", "NCUA", "EQUAL HOUSING", "AMAZON", "AMZN", "WALMART", "WAL-MART", "TARGET", "COSTCO",
    "KROGER", "STARBUCKS", "MCDONALD", "NETFLIX", "SPOTIFY", "APPLE.COM", "GOOGLE", "UBER", "LYFT",
    "EXXON", "SHELL OIL", "CHEVRON",
]
...
expected[1] += [
    S.BANK_NAME, S.BANK_TAGLINE, S.CUSTOMER_SERVICE_PHONE, S.ACCOUNT_HOLDER,
    S.HOLDER_ADDRESS_LINES[1], S.HOLDER_ADDRESS_LINES[2], S.ACCOUNT_NUMBER, S.CARD_LAST4,
    S.ACCOUNT_TYPE, S.PERIOD_LABEL, S.STATEMENT_DATE, S.CUSTOMER_EMAIL, "Statement Delivery",
    "Account Summary", "Beginning Balance",
    "Ending Balance", "Deposits and Other Credits",
    S.money(S.BEGINNING_BALANCE), S.money(S.TOTAL_CREDITS), S.money(S.TOTAL_WITHDRAWALS),
    S.money(S.TOTAL_CHECKS), S.money(S.TOTAL_FEES), S.money(S.ENDING_BALANCE),
]
for t in S.credits():
    expected[1] += list(t.desc_lines)
# page 2 — continuation furniture / withdrawals / checks / fees
expected[2] += [
    S.ACCOUNT_MASKED, "Page 2 of 3", S.PERIOD_LABEL, "Withdrawals and Other Subtractions",
    "Total Withdrawals and Other Subtractions", S.money(S.TOTAL_WITHDRAWALS), "Checks Paid",
    S.money(S.TOTAL_CHECKS), "Service Fees", "Monthly Maintenance Fee",
    "Fees Year-to-Date", S.money(S.FEES_YTD), S.money(S.TOTAL_FEES),
]
...
expected[3] += [
    "Page 3 of 3", "Daily Ending Balance", S.REG_E_HEADING, S.CUSTOMER_SERVICE_PHONE,
    "60 days", "10 business days", S.money(S.ENDING_BALANCE),
]
...
check(not missing, "2. all manifest strings present per page",
      f"{len(missing)} missing: {missing[:6]}" if missing else f"{sum(len(v) for v in expected.values())} strings OK")
...
check(font_lines and not bad_fonts, "4. only Inter, embedded+subset",
...
check(not hits_text and not hits_mod, "5. denylist clean (text + module)",
...
check(not bad_phones, "6. phones in reserved range", f"bad: {bad_phones}" if bad_phones else "1-800-555-0199 only")

# =================================================================================================
# 7. NO standalone 9-digit number (max digit run length != 9)
runs9 = [r for r in re.findall(r"\d+", full_text) if len(r) == 9]
check(not runs9, "7. no standalone 9-digit number", f"found: {runs9}" if runs9 else "none (routing omitted)")

# =================================================================================================
# 8. NO SSN-shaped string
ssn = re.findall(r"\b\d{3}-\d{2}-\d{4}\b", full_text) + re.findall(r"\b\d{3}\s\d{2}\s\d{4}\b", full_text)
...
arith_ok = (
    ind_credits == S.TOTAL_CREDITS and ind_wd == S.TOTAL_WITHDRAWALS and ind_ck == S.TOTAL_CHECKS
    and ind_fee == S.TOTAL_FEES and ind_end == S.ENDING_BALANCE
    and ind_daily == S.daily_ending_balances() and ind_daily[-1][1] == S.ENDING_BALANCE
)
check(arith_ok, "9. arithmetic consistent (summary + daily fold)",
...
emb_dates = re.findall(r"(?:CHECKCARD|ATM WITHDRAWAL)\s+(\d{2})(\d{2})", " ".join(
    ln for t in S.TRANSACTIONS for ln in t.desc_lines))
bad_emb = [f"{mm}{dd}" for mm, dd in emb_dates if mm != "05" or not (1 <= int(dd) <= 31)]
check(not bad_days and not bad_emb, "10. txn dates within period",
...
check(not md_bad, "11. metadata == locked values", f"mismatch: {md_bad}" if md_bad else "Title/Author/Subject/Creator OK")
