...
def need_cv2():
    try:
        ...
        sys.exit("register_capture needs opencv-python in the sd venv (admitted tooling, D12-36): "
...
def need_pdfium():
    try:
        ...
        sys.exit("the synthetic subcommand needs pypdfium2 in the sd venv (admitted tooling, D12-36)")
    return pdfium
...
class Masters:
    def __init__(self, sd_root: Path):
        ...
        self.sha = {"pdf": sha256_path(self.pdf), "marks": sha256_path(self.marks_path),
                    "ground_truth": sha256_path(self.gt_path)}
        if self.marks.get("sha256") != self.sha["pdf"]:
            sys.exit(f"marks sidecar names sha {self.marks.get('sha256')} but the PDF hashes "
        ...
        self.markers_by_page = {int(p["page"]): {int(k): [float(v) for v in q]
                                                 for k, q in p["markers"].items()}
                                for p in self.marks["pages"]}
        self.page_meta = {}
        for imp in self.marks["imported_pages"]:
            self.page_meta[int(imp["master_page"])] = {
                "exhibit": imp["exhibit"], "class": "C1", "designed_doctype": None,
                "imported_from_packet_page": int(imp["packet_page"])}
        for dr in self.marks["drawn_pages"]:
            self.page_meta[int(dr["master_page"])] = {
                "exhibit": dr["exhibit"], "class": dr["class"],
                "designed_doctype": dr["designed_doctype"], "imported_from_packet_page": None}
    ...
    def bbox_polygon_pt(self, bbox: list[float]) -> list[tuple[float, float]]:
        ...
        return [(x0 * w, h - y1 * h), (x1 * w, h - y1 * h), (x1 * w, h - y0 * h), (x0 * w, h - y0 * h)]
...
def _marker_grid(marker_id: int) -> np.ndarray:
    ...
    return np.array([[cells[r][c] for c in range(1, 1 + A.GRID)] for r in range(1, 1 + A.GRID)], np.uint8)
...
# ---------------------------------------------------------------------------------------------
def make_params(cv2, refine: str, *, error_correction_rate: float | None, min_perimeter_rate: float):
    p = cv2.aruco.DetectorParameters()
    p.cornerRefinementMethod = {
        "none": cv2.aruco.CORNER_REFINE_NONE, "subpix": cv2.aruco.CORNER_REFINE_SUBPIX,
        "contour": cv2.aruco.CORNER_REFINE_CONTOUR, "apriltag": cv2.aruco.CORNER_REFINE_APRILTAG,
    }[refine]
...
def choose_candidates(found: dict[int, list[np.ndarray]]) -> tuple[dict[int, np.ndarray], list[int]]:
...
def register_image(cv2, masters: Masters, gray: np.ndarray, *, refine: str = "subpix",
                   max_error_px: float = 2.0, max_corner_error_px: float = 3.0, min_markers: int = 4,
                   ransac_px: float = 3.0, max_correction_cap: int = 2, min_perimeter_rate: float = 0.01,
    ...
    out: dict = {"width_px": int(W_img), "height_px": int(H_img), "accepted": False, "reason": None,
                 "refine": refine, "max_error_px": max_error_px, "max_corner_error_px": max_corner_error_px,
                 "min_markers": min_markers, "ransac_px": ransac_px}
    full = cv2.aruco.getPredefinedDictionary(cv2.aruco.DICT_4X4_100)
    found_full, rejected_full = detect(cv2, gray, full, make_params(
        cv2, refine, error_correction_rate=None, min_perimeter_rate=min_perimeter_rate))
    ...
    if restrict:
        ...
        sub = cv2.aruco.Dictionary(full.bytesList[expected[0]:expected[-1] + 1].copy(),
                                   full.markerSize, maxcorr)
        found_sub, rejected_sub = detect(cv2, gray, sub, make_params(
        ...
        out["restricted_pass"] = {"ids": sorted(found_sub), "rejected_candidates": rejected_sub,
                                  "min_hamming_distance": dmin, "max_correction_bits": maxcorr,
    ...
    if len(chosen) < min_markers:
        out["reason"] = f"{len(chosen)}/{MARKERS_PER_PAGE} fiducials of page {page} detected (< {min_markers})"
        return out
    ...
    out["reprojected_corners"] = {str(mid): proj[4 * k:4 * k + 4].round(3).tolist()
                                  for k, mid in enumerate(sorted(chosen))}
    ...
    out["error_px_at300"] = round(rms * 300.0 / out["dpi_effective"], 4) if out["dpi_effective"] else None
    if rms > max_error_px:
        out["reason"] = f"reprojection RMS {rms:.3f} px > {max_error_px} px"
        return out
    if float(resid.max()) > max_corner_error_px:
        out["reason"] = (f"worst corner {float(resid.max()):.3f} px > {max_corner_error_px} px "
...
def transform_rows(cv2, masters: Masters, page: int, H: np.ndarray, W: int, Hpx: int) -> tuple[list[dict], int]:
    ...
    def project(bbox):
        ...
        return ([[round(x, 6), round(y, 6)] for x, y in norm],
                [round(v, 6) for v in clamped], inside, [round(x, 3) for x in img.reshape(-1).tolist()])
...
def draw_overlay(cv2, gray: np.ndarray, reg: dict, rows: list[dict], label: str, max_px: int) -> np.ndarray:
    Hpx, W = gray.shape[:2]
    scale = min(1.0, max_px / float(max(W, Hpx)))
    bgr = cv2.cvtColor(gray, cv2.COLOR_GRAY2BGR)
    if scale < 1.0:
        bgr = cv2.resize(bgr, (int(round(W * scale)), int(round(Hpx * scale))), interpolation=cv2.INTER_AREA)
    t = max(1, int(round(bgr.shape[1] / 900.0)))

    def pts(seq):
        return np.array([[int(round(x * scale)), int(round(y * scale))] for x, y in seq], np.int32).reshape(-1, 1, 2)
    ...
    for mid, quad in reg.get("markers", {}).items():
        ...
        x, y = quad[0]
        cv2.putText(bgr, mid, (int(x * scale) + 3, int(y * scale) - 3), cv2.FONT_HERSHEY_SIMPLEX,
                    0.5 * max(1, t), (0, 200, 0), t)
    for _mid, quad in reg.get("reprojected_corners", {}).items():
        for x, y in quad:
            cv2.circle(bgr, (int(round(x * scale)), int(round(y * scale))), 2 * t + 1, (0, 0, 255), -1)
    cv2.rectangle(bgr, (0, 0), (bgr.shape[1], 22 * t + 10), (255, 255, 255), -1)
    cv2.putText(bgr, label, (6, 18 * t + 4), cv2.FONT_HERSHEY_SIMPLEX, 0.6 * max(1, t), (0, 0, 0), t)
    return bgr
...
def load_image(cv2, path: Path, stage_dir: Path) -> tuple[np.ndarray, Path, dict]:
    ...
    meta = {"source": str(path), "source_sha256": sha256_path(path), "exif_orientation": None,
            "re_encoded": False}
    embed = path
    ...
    if gray is None:
        sys.exit(f"cannot read image {embed} (HEIC? set the iPhone to Most Compatible, or convert first)")
    meta["image"] = str(embed)
    meta["image_sha256"] = meta["source_sha256"] if embed == path else sha256_path(embed)
    meta["format"] = "jpeg" if embed.suffix.lower() in {".jpg", ".jpeg"} else embed.suffix.lower().lstrip(".")
...
def assemble_pdf(pages: list[dict], title: str, doc_id: bytes) -> bytes:
    ...
    buf = io.BytesIO()
    first = pages[0]
    c = canvas.Canvas(buf, pagesize=(first["page_w_pt"], first["page_h_pt"]), invariant=1, pageCompression=1)
    for p in pages:
        c.setPageSize((p["page_w_pt"], p["page_h_pt"]))
        c.drawImage(p["image"], 0, 0, width=p["page_w_pt"], height=p["page_h_pt"],
...
def condition_of(leg: str, reg: dict, path: Path, k: int, group_size: int, photo_order: list[str]) -> str | None:
...
def cmd_register(args) -> int:
    ...
    legs = args.legs.split(",") if args.legs else sorted(
        d.name for d in capture_dir.iterdir() if d.is_dir() and any(
    ...
    derive_pages = [int(s) - 1 for s in args.derive_rot90.split(",") if s.strip()] if args.derive_rot90 else []
    ...
    legs = [x for x in legs if x != "rotated"] + (["rotated"] if "rotated" in legs or derive_pages else [])

    reg_params = {"refine": args.refine, "max_error_px": args.max_error_px,
                  "max_corner_error_px": args.max_corner_error_px, "min_markers": args.min_markers,
                  "ransac_px": args.ransac_px, "max_correction_cap": args.max_correction_bits,
                  "min_perimeter_rate": args.min_perimeter_rate, "restrict": not args.no_restrict}
    index = {"tool": TOOL, "version": TOOL_VERSION, "sd_sha": git_describe(REPO),
             "masters": {"pdf": str(masters.pdf), **masters.sha, "page_count": masters.page_count},
             "capture_dir": str(capture_dir), "stamp": stamp, "params": reg_params,
             "photo_order": photo_order, "legs": {}}
    manifest_rows = []
    summary_lines = []
    ...
    for leg in legs:
        ...
        if derive_pages and leg == "rotated":
            ...
            for p in derive_pages:
                ...
                if not srcs:
                    print(f"  ! derive-rot90: page {p + 1:02d} has no accepted {derive_source} image; skipped")
                    continue
                src = srcs[0]
                ...
                write_json(leg_dir / f"derived-rot90-p{p + 1:02d}.provenance.json", {
                    "derived_from": src["image"], "source_sha256": src["image_sha256"],
                    ...
                    "why": "90-degree feeds are physically impossible on letter-width hardware (IM-30 adaptation, "
                           "pre-cleared 2026-08-25); derived from the 150 DPI scan leg", "saved_as": "PNG (lossless)"})
                print(f"  derived {dst.name} from {Path(src['image']).name}")
        files = sorted(f for f in leg_dir.iterdir() if f.is_file() and f.suffix.lower() in IMAGE_EXTS)
        if not files:
        ...
        for f in files:
            ...
            hint = page_hint(f)
            reg["page_hint"] = hint
            reg["page_hint_agrees"] = (None if hint is None or reg.get("page") is None else hint == reg["page"])
            if reg["page_hint_agrees"] is False:
                print(f"  ! {f.name}: filename says page {hint + 1:02d}, fiducials say {reg['page'] + 1:02d} "
                      f"(fiducials win)")
            if reg["accepted"]:
                page = reg["page"]
                gt_rows, oof = transform_rows(cv2, masters, page, reg["homography"], reg["width_px"], reg["height_px"])
                ...
                write_json(gt_path, {"schema_version": 1, "tool": TOOL, "version": TOOL_VERSION,
                                     "leg": leg, "image": str(embed), "image_sha256": reg["image_sha256"],
                                     "master_page": page, "registration": {k: reg[k] for k in (
                                         "homography", "homography_frame", "homography_error_px",
                                         "homography_error_max_px", "dpi_effective", "rotation_deg", "mirrored",
                                         "markers_detected", "corner_count", "ransac_inliers")},
                                     "bbox_origin": "bottom-left", "occurrences": gt_rows,
                                     "carried_stmt": pending})
                reg["gt_json"] = str(gt_path)
                label = (f"{leg} {f.name} -> master p{page + 1:02d} rms {reg['homography_error_px']:.2f} px "
                ...
                print(f"  {f.name}: page {page + 1:02d}  markers {reg['markers_detected']}/4  "
        ...
        for page, regs in by_page.items():
            ...
            for k, r in enumerate(regs):
                ...
                if leg == "photo" and len(regs) != len(photo_order):
                    print(f"  ! photo page {page + 1:02d} has {len(regs)} shots, expected "
                          f"{len(photo_order)} ({','.join(photo_order)}); labeled cond1..")

        leg_summary = {"kind": spec["kind"], "nominal_dpi": spec["dpi"], "features": spec["features"],
                       "synthetic": spec["synthetic"], "images": len(rows_out),
                       "accepted": sum(1 for r in rows_out if r["accepted"]),
                       "rejected": [r["file"] + ": " + str(r["reason"]) for r in rows_out if not r["accepted"]],
        ...
        if args.emit_pdf and by_page:
            ...
            for cond, regs in conds.items():
                ...
                for idx, r in enumerate(regs):
                    dpi_nom = spec["dpi"]
                    dpi_x = dpi_y = float(dpi_nom) if (dpi_nom and args.pdf_dpi == "nominal") else None
                    if dpi_x is None:
                        dpi_x, dpi_y = (float(v) for v in r["dpi_effective_xy"])
                    page_w_pt = r["width_px"] * 72.0 / dpi_x
                    page_h_pt = r["height_px"] * 72.0 / dpi_y
                    pages_spec.append({"image": r["image"], "page_w_pt": page_w_pt, "page_h_pt": page_h_pt})
                    meta = masters.page_meta[r["page"]]
                    page_table.append({"page": idx, "master_page": r["page"], "exhibit": meta["exhibit"],
                                       "class": meta["class"], "designed_doctype": meta["designed_doctype"],
                                       ...
                                       "dpi_effective": r["dpi_effective"], "rotation_deg": r["rotation_deg"],
                                       ...
                                       "markers_detected": r["markers_detected"], "condition": cond})
                ...
                title = f"{masters.gt['set_id']} capture {stamp} {name} ({spec['kind'] or 'capture'})"
                ...
                sidecar = {
                    "schema_version": 1, "packet": masters.gt["packet"], "generator": TOOL,
                    "set_id": masters.gt["set_id"], "document": pdf_path.name, "sha256": sha256_bytes(pdf),
                    "masters": {"document": MASTERS_PDF, **masters.sha},
                    "capture": {"leg": leg, "condition": cond, "kind": spec["kind"], "stamp": stamp,
                                "nominal_dpi": spec["dpi"], "synthetic": spec["synthetic"],
                                "features": spec["features"], "page_dpi_rule": args.pdf_dpi,
                                "registration": {"tool": TOOL, "version": TOOL_VERSION, **reg_params},
                                "pages": page_table},
                    "page_count": len(page_table), "bbox_origin": "bottom-left",
                    "page_size_pt": list(sizes.pop()) if len(sizes) == 1 else None,
                    "imported": [{"page": p["page"], "master_page": p["master_page"], "exhibit": p["exhibit"],
                                  "packet_page": p["imported_from_packet_page"]}
                                 for p in page_table if p["imported_from_packet_page"] is not None],
                    "exhibits": [{"name": p["exhibit"], "base_page": p["page"], "pages": 1, "class": p["class"],
                                  "designed_doctype": p["designed_doctype"], "master_page": p["master_page"]}
                                 for p in page_table if p["imported_from_packet_page"] is None],
                    "occurrences": gt_rows_all,
                    "carried_stmt": pending_all,
                    "notes": ("Ground truth of the capture masters carried through the fiducial homography "
                ...
                if problems:
                    print(f"  ! {name}: {len(problems)} ground-truth schema problems (first: {problems[0]})")
                doc = {"name": name, "condition": cond, "pdf": str(pdf_path), "pdf_sha256": sidecar["sha256"],
                       "pdf_bytes": len(pdf), "pages": len(page_table), "gt": str(gt_path),
                       "gt_sha256": sha256_path(gt_path), "gt_rows": len(gt_rows_all),
                ...
                manifest_rows.append({
                    ...
                    "sha256": sidecar["sha256"], "pages": len(page_table), "class": None,
                    "capture": spec["kind"], "features": spec["features"],
                    "gt": f"<DOCS_ROOT placement ruled at step 4>/{gt_path.name}", "gt_kind": "bbox",
                    "leg_applicability": ["ocr"], "source": "external",
                    "provenance": {"generator": TOOL, "seed": None, "variant": name, "capture_leg": leg,
                                   "condition": cond, "capture_stamp": stamp, "synthetic": spec["synthetic"],
                                   ...
                                   "note": ("class is null because one document spans C1..C4 (per-page classes "
                                            "in page_classes; the L-19 null precedent); polygons ride beside "
                                            "bbox in the sidecar")},
                })
                print(f"  wrote {pdf_path.name} ({len(pdf):,} bytes, {len(page_table)} pages) + {gt_path.name} "
    ...
    versions = {"python_sd_venv": platform.python_version(), "opencv-python": pkg_version("opencv-python"),
                "opencv-python-headless": pkg_version("opencv-python-headless"), "cv2": cv2.__version__,
                "numpy": np.__version__, "pypdfium2": pkg_version("pypdfium2"), "pymupdf": pkg_version("pymupdf"),
                "pillow": pkg_version("pillow"), "reportlab": pkg_version("reportlab"), "pypdf": pkg_version("pypdf")}
    run = {
        "baseline_sha": args.baseline_sha, "dp_sha": args.dp_sha, "sd_sha": index["sd_sha"],
        "date": datetime.now().astimezone().isoformat(timespec="seconds"),
        "host": f"{platform.node()} / macOS {platform.mac_ver()[0]} ({platform.machine()})",
        "xcode_build": "n/a (no Swift run)", "sim": None, "device": None,
        "emitter": f"{TOOL} register (v{TOOL_VERSION})",
        "env": {}, "inputs": [{"manifest_id": None, "path": str(masters.pdf), "sha256": masters.sha["pdf"]},
                              {"manifest_id": None, "path": str(masters.marks_path), "sha256": masters.sha["marks"]},
                              {"manifest_id": None, "path": str(masters.gt_path), "sha256": masters.sha["ground_truth"]}]
        + [{"manifest_id": None, "path": r["image"], "sha256": r["image_sha256"]}
           for leg in index["legs"].values() for r in leg["rows"]],
        "n_runs": 1, "params": reg_params, "capture_dir": str(capture_dir), "legs": list(index["legs"]),
        "tool_versions": versions, "notes": args.note or "",
    }
    write_json(out / "run.json", run)
    head = ["# SUMMARY (tool draft) -- register_capture " + stamp, "",
    ...
    tail = ["", "## documents"] + [
        ...
        for leg in index["legs"].values() for d in leg["documents"]]
...
def cmd_synthetic(args) -> int:
    ...
    pages = [int(s) - 1 for s in args.pages.split(",")] if args.pages else list(range(masters.page_count))
    doc = pdfium.PdfDocument(str(masters.pdf))
    for dpi in dpis:
        leg = f"synth{dpi}" + (f"-rot{args.rotate}" if args.rotate else "") + (args.suffix or "")
        leg_dir = out_dir / leg
        leg_dir.mkdir(parents=True, exist_ok=True)
        prov = {"tool": TOOL, "version": TOOL_VERSION, "renderer": f"pypdfium2 {pkg_version('pypdfium2')}",
                "masters": {"document": MASTERS_PDF, **masters.sha}, "dpi": dpi, "grayscale": True,
                "format": args.format, "jpeg_quality": args.jpeg_quality if args.format == "jpeg" else None,
                "rotate_deg": args.rotate, "clip_left_pt": args.clip_left_pt, "files": []}
        for i in pages:
            page = doc[i]
            ...
            prov["files"].append({"file": name, "master_page": i, "sha256": sha256_path(path),
...
def build_parser() -> argparse.ArgumentParser:
    ...
    s.add_argument("--pages", default=None, help="1-indexed master pages, comma-separated (default all)")
    ...
    s.add_argument("--clip-left-pt", type=float, default=0.0,
    ...
    r = sub.add_parser("register", help="register every leg under a capture dir into an evidence run dir")
    r.add_argument("--sd-root", default=str(REPO))
    r.add_argument("--capture-dir", required=True)
    r.add_argument("--out", required=True)
    r.add_argument("--legs", default=None, help="comma-separated subset (default: every subdir with images)")
    r.add_argument("--stamp", default=None, help="capture stamp for ids (default: the capture dir name)")
    r.add_argument("--refine", choices=["none", "subpix", "contour", "apriltag"], default="subpix")
    r.add_argument("--max-error-px", type=float, default=2.0, help="reject above this reprojection RMS ([R06])")
    r.add_argument("--max-corner-error-px", type=float, default=3.0,
    ...
    r.add_argument("--max-correction-bits", type=int, default=2,
    ...
    r.add_argument("--no-restrict", action="store_true", help="skip the page-restricted sub-dictionary pass")
    r.add_argument("--emit-pdf", action="store_true", help="assemble one PDF + GT sidecar per (leg, condition)")
    r.add_argument("--pdf-dpi", choices=["nominal", "effective"], default="nominal",
    ...
    r.add_argument("--derive-rot90", default=None, help="1-indexed pages to derive by rot90 (e.g. 2,10)")
