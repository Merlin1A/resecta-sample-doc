...
def build_pdf(objects: list[tuple[int, bytes]], root: int, info: int | None = None,
              version: bytes = b"1.4") -> bytes:
...
def page_pdf(content: bytes, *, extra_page: bytes = b"", extra_objects: list[tuple[int, bytes]] | None = None,
             catalog_extra: bytes = b"", resources: bytes = b"/Font << /F1 5 0 R >>",
             info: int | None = None, version: bytes = b"1.4",
             content_dict: bytes = b"") -> bytes:
    """1-page skeleton: 1 catalog / 2 pages / 3 page / 4 contents / 5 Helvetica (+extras)."""
    objs = [
        (1, b"<< /Type /Catalog /Pages 2 0 R" + catalog_extra + b" >>"),
        (2, b"<< /Type /Pages /Kids [3 0 R] /Count 1 >>"),
        (3, b"<< /Type /Page /Parent 2 0 R /MediaBox [0 0 %d %d] /Contents 4 0 R "
...
def anchor(label: str) -> bytes:
    return (text_op(72, 730, 14, f"Synthetic planted-leak fixture -- {label}")
            + text_op(72, 706, 10, "Fictional content; no real PII. Oracle-calibration corpus (T2.2)."))
...
def emit(fixture: str, pdf: bytes, plants: list[dict], *, clean: bool = False,
         notes: str = "") -> None:
...
def plant(surface: str, term: str, expected_legs: list[str], *, hidden_class: str | None = None,
          expect_visible_in_output: bool, page: int = 0, bbox: list[float] | None = None,
          notes: str = "") -> dict:
    return {"surface": surface, "term": term, "hidden_class": hidden_class,
            "expect_visible_in_output": expect_visible_in_output,
            "expected_legs": expected_legs, "page": page, "bbox": bbox, "notes": notes}


def fx_visible() -> None:
    t = "PLANT-VISIBLE-01"
    c = anchor("visible text control") + text_op(72, 400, 18, f"Reference value: {t}")
    emit("planted-visible", page_pdf(c),
         [plant("visible_text", t, ["o1"], expect_visible_in_output=True,
...
def fx_wow() -> None:
    t = "PLANT-WOW-01"
    c = (anchor("white-on-white hidden text")
         + b"1 g\n" + text_op(72, 400, 18, t) + b"0 g\n")
    emit("planted-hidden-wow", page_pdf(c),
         [plant("hidden_text", t, ["o1"], hidden_class="white_on_white",
                expect_visible_in_output=False, bbox=nrect(72, 396, 190, 22),
...
def fx_tr3() -> None:
    ...
    emit("planted-hidden-tr3", page_pdf(c),
         [plant("hidden_text", t, ["o1"], hidden_class="tr3_invisible",
                expect_visible_in_output=False, bbox=nrect(72, 396, 210, 22),
...
def fx_box() -> None:
    t = "PLANT-BOX-01"
    c = (anchor("opaque painted box over text")
         + text_op(72, 400, 18, t)
         + b"0 g 66 392 220 30 re f\n")
    emit("planted-hidden-box", page_pdf(c),
         [plant("hidden_text", t, ["o1"], hidden_class="opaque_box",
                expect_visible_in_output=False, bbox=nrect(72, 396, 190, 22),
...
def fx_ocg() -> None:
    ...
    emit("planted-hidden-ocg", pdf,
         [plant("hidden_text", t, ["o2", "structure"], hidden_class="ocg_off",
                expect_visible_in_output=False, bbox=nrect(72, 396, 200, 22),
...
def fx_hex() -> None:
    ...
    c = (anchor("hex-string text operand")
         + b"BT /F1 18 Tf 72 400 Td <" + hexs + b"> Tj ET\n")
    emit("planted-hex-string", page_pdf(c),
         [plant("hex_string", t, ["o1", "o2"], expect_visible_in_output=True,
...
def fx_flate() -> None:
    t = "PLANT-FLATE-01"
    inner = anchor("Flate-compressed content stream") + text_op(72, 400, 18, f"Compressed value: {t}")
    emit("planted-flate-content", page_pdf(zlib.compress(inner, 9),
                                           content_dict=b"/Filter /FlateDecode"),
         [plant("flate_stream", t, ["o1", "o2"], expect_visible_in_output=True,
...
def fx_objstm() -> None:
    ...
    objs.append((6, stream_obj(
        b"/Type /ObjStm /N 1 /First %d /Filter /FlateDecode" % first, objstm_data)))
    ...
    data = b"".join(bytes([tp]) + off.to_bytes(4, "big") + g.to_bytes(2, "big")
                    for tp, off, g in rows)
    xref_dict = (b"/Type /XRef /Size 9 /W [1 4 2] /Index [0 9] /Root 1 0 R /Info 7 0 R"
                 b" /Length %d" % len(data))
    body += b"8 0 obj\n<< " + xref_dict + b" >>\nstream\n" + data + b"\nendstream\nendobj\n\n"
    body += b"startxref\n%d\n%%%%EOF" % xref_off
    emit("planted-objstm", body,
         [plant("object_stream", t, ["o2", "structure"], expect_visible_in_output=False,
...
def fx_inline_image() -> None:
    ...
    c = (anchor("inline image (BI/ID/EI)")
         + b"q 450 0 0 75 81 380 cm\nBI /W 600 /H 100 /CS /G /BPC 8 /F /AHx ID\n"
         + ahx(img.tobytes())
         + b"\nEI\nQ\n")
    emit("planted-inline-image", page_pdf(c),
         [plant("inline_image", t, ["o3"], expect_visible_in_output=True,
...
def fx_annot_ap() -> None:
    ...
    pdf = page_pdf(
        ...
        extra_objects=[
            (6, b"<< /Type /Annot /Subtype /FreeText /Rect [72 380 372 416] "
                b"/Contents (synthetic note) /DA (/F1 16 Tf 0 g) /F 4 /AP << /N 7 0 R >> >>"),
            (7, stream_obj(b"/Type /XObject /Subtype /Form /BBox [0 0 300 36] "
                           b"/Resources << /Font << /F1 5 0 R >> >>", ap)),
        ])
    emit("planted-annot-ap", pdf,
         [plant("annotation_ap", t, ["o2", "structure"], expect_visible_in_output=True,
...
def fx_annot_contents() -> None:
    ...
    pdf = page_pdf(
        ...
        extra_page=b" /Annots [6 0 R]",
        extra_objects=[
            (6, b"<< /Type /Annot /Subtype /Square /Rect [72 380 272 420] "
                b"/Contents (" + esc(t) + b") /F 4 >>"),
        ])
    emit("planted-annot-contents", pdf,
         [plant("annotation_contents", t, ["o2", "structure"], expect_visible_in_output=False,
...
def fx_xmp() -> None:
    ...
    emit("planted-xmp", pdf,
         [plant("xmp_metadata", t, ["o2", "structure"], expect_visible_in_output=False,
...
def fx_info() -> None:
    ...
    pdf = page_pdf(
        ...
        info=6,
        extra_objects=[(6, b"<< /Title (Synthetic Info fixture) /Subject (" + esc(t) + b") "
                           b"/Author (Fixture Author) >>")])
    emit("planted-info", pdf,
         [plant("info_dict", t, ["o2", "structure"], expect_visible_in_output=False,
...
def fx_exif() -> None:
    ...
    pdf = page_pdf(
        anchor("EXIF ImageDescription in a page JPEG")
        + b"q 400 0 0 300 106 300 cm /Im1 Do Q\n",
        resources=b"/Font << /F1 5 0 R >> /XObject << /Im1 6 0 R >>",
        extra_objects=[(6, stream_obj(
            b"/Type /XObject /Subtype /Image /Width 400 /Height 300 "
            b"/ColorSpace /DeviceGray /BitsPerComponent 8 /Filter /DCTDecode", jpg))])
    emit("planted-exif", pdf,
         [plant("exif", t, ["o2"], expect_visible_in_output=False,
...
def fx_acroform() -> None:
    ...
    pdf = page_pdf(
        ...
        extra_page=b" /Annots [6 0 R]",
        extra_objects=[
            (6, b"<< /Type /Annot /Subtype /Widget /FT /Tx /T (field1) /V (" + esc(t) + b") "
                b"/Rect [72 380 372 410] /P 3 0 R /F 4 /DA (/F1 12 Tf 0 g) >>"),
        ])
    emit("planted-acroform-v", pdf,
         [plant("acroform_v", t, ["o2", "structure"], expect_visible_in_output=False,
...
def fx_outlines() -> None:
    ...
    emit("planted-outlines", pdf,
         [plant("outlines", t, ["o2", "structure"], expect_visible_in_output=False,
...
def fx_thumb() -> None:
    ...
    pdf = page_pdf(
        ...
        extra_objects=[(6, stream_obj(
            ...
            ahx(img.tobytes())))])
    emit("planted-thumb", pdf,
         [plant("thumb", t, ["structure"], expect_visible_in_output=False,
...
def fx_prev_revision() -> None:
    t = "PLANT-PREVREV-01"
    rev1_content = (anchor("prior revision (REAL /Prev chain)")
                    + text_op(72, 400, 18, f"Original value: {t}"))
    objs = [
        (1, b"<< /Type /Catalog /Pages 2 0 R >>"),
        (2, b"<< /Type /Pages /Kids [3 0 R] /Count 1 >>"),
        (3, b"<< /Type /Page /Parent 2 0 R /MediaBox [0 0 612 792] /Contents 4 0 R "
    ...
    rev2_content = (anchor("prior revision (REAL /Prev chain)")
                    + text_op(72, 400, 18, "Original value: [withdrawn in revision 2]"))
    upd = b"\n"
    ...
    emit("planted-prior-revision", rev1 + upd,
         [plant("prior_revision", t, ["o2", "structure"], expect_visible_in_output=False,
...
def fx_embedded_file() -> None:
    ...
    emit("planted-embedded-file", pdf,
         [plant("embedded_file", t, ["o2", "structure"], expect_visible_in_output=False,
...
def fx_javascript() -> None:
    ...
    emit("planted-javascript", pdf,
         [plant("javascript", t, ["o2", "structure"], expect_visible_in_output=False,
...
def fx_text_outlines_probe() -> str:
    ...
    for args in (["mutool", "convert", "-O", "text=path", "-o", str(out), str(src)],):
        ...
        if font_rows <= 0 and b"PLANT-VISIBLE-01" not in text.stdout:
            pdf = out.read_bytes()
            emit("planted-text-outlines", pdf,
                 [plant("text_as_outlines", "PLANT-VISIBLE-01", ["o3"],
            ...
            return "emitted"
        out.unlink(missing_ok=True)
    SKIPPED.append({"fixture": "planted-text-outlines", "surface": "text_as_outlines",
...
def clean_docs() -> None:
    c1 = (anchor("clean control letter")
    ...
    emit("clean-letter", page_pdf(c1), [], clean=True,
         notes="benign visible text only; no metadata beyond the skeleton")

    c2 = anchor("clean control table")
    y = 640
    for i, (k, v) in enumerate([("Row", "Amount"), ("AA-101", "12.00"), ("AA-102", "7.50"),
                                ("AA-103", "19.25"), ("AA-104", "3.10")]):
        c2 += text_op(90, y - i * 24, 11, k) + text_op(300, y - i * 24, 11, v)
    pdf2 = page_pdf(c2, info=6,
                    extra_objects=[(6, b"<< /Producer (Synthetic Fixture Producer) "
                                       b"/Creator (build_planted_corpus) >>")])
    emit("clean-table", pdf2, [], clean=True,
    ...
    pdf3 = page_pdf(
        ...
        extra_objects=[(6, stream_obj(
            b"/Type /XObject /Subtype /Image /Width 400 /Height 300 "
            b"/ColorSpace /DeviceGray /BitsPerComponent 8 /Filter /DCTDecode", jpg))])
    emit("clean-image", pdf3, [], clean=True,
         notes="benign JPEG page, EXIF-free")
...
def main() -> None:
    ...
    print(f"[planted] {len(manifest['rows'])} planted + {len(manifest['clean'])} clean fixtures, "
